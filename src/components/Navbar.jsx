...
const Navbar = () => {
    ...
    useEffect(() => {
        const checkedElements = items.filter(element => element.completed === true);
    ...
    return (
        <div className="flex flex-col bg-[#1d1d1d] p-0 gap-6">
            <div className="bg-[#69a4df] text-white p-1 rounded-2xl mt-8 ml-5 mr-5 flex items-center">
                <img src={circles} alt='circles' className="w-20 h-28 mr-4" />
                <div>
                    <h2 className="text-lg font-semibold">Your Daily Goal Almost Done</h2>
                    <p className="text-sm">{checked ? `${checked} of ${items.length} Completed` : '0 of 0 Completed'}</p>
                    <div className="relative w-full mt-2 bg-white h-2.5 rounded-full">
                        <div className="absolute top-0 left-0 bg-[#0b121a] h-full rounded-full" style={{ width: `${(checked / items.length) * 100}%` }} />
                    </div>
                    <div className="text-right text-sm font-semibold mt-1">{checked ? `${(checked / items.length) * 100}%` : '0%'}</div>
                </div>
            </div>
            <div className="flex-1 overflow-y-auto p-4 bg-[#1d1d1d]">
                <h3 className="text-lg font-semibold text-white mb-4 flex justify-between m-2 ">Today's Goal <img src={heartgif} alt='gif' /></h3>

                <div className="space-y-4">
                    {items.length > 0 ? (
                        items.map(element => (
                            <div key={element.id} className={`flex items-center justify-between bg-zinc-700 p-4 rounded-xl ${element.completed && ' text-red-50'}`}>
                                <div className="text-purple-600 text-2xl"><img src={workout} alt='workout' className='size-8 ml-3 mr-5' /></div>
                                <span className="text-white ml-2 flex-1">{element.task}</span>
                                <input type="checkbox" className=" form-checkbox h-6 w-6 text-green-500 rounded-sm border-gray-300 focus:ring-green-500 " checked={element.completed} onChange={() => handleCheck(element.id)} />
                            </div>
                        ))
                    ) : (
                        <div>No Tasks</div>
           ...
           <div className="text-white text-2xl"><img src={ first} alt='page1' /> </div>
				<div className="text-white text-2xl"><img src={ second} alt='page2'/></div>
                <div className="text-white text-2xl"><img src={ third} alt='page3'/></div>
                <div className="text-white text-2xl"><img src={ fourth} alt='page4'/></div>
            </div>
        </div>
    );
};
